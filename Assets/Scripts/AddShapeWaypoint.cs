...
public class AddShapeWaypoint : MonoBehaviour
{
	...
	private void Start()
	{
		...
		mapManager = GetComponent<CreateMapSample1>();
	...
	bool HitTestWithResultType(ARPoint point, ARHitTestResultType resultTypes)
	{
		...
		if (hitResults.Count > 0)
		{
			foreach (var hitResult in hitResults)
			{
				...
				Matrix4x4? placenoteTransform = LibPlacenote1.Instance.ProcessPose(worldTransform);

				Vector3 hitPosition = PNUtility1.MatrixOps.GetPosition(placenoteTransform.Value);
				Quaternion hitRotation = PNUtility1.MatrixOps.GetRotation(placenoteTransform.Value);
