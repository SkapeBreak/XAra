...
namespace EditorCoroutines {
	...
	[System.Serializable]
	public class WWWStatus1
	{
		public bool status;
	}
		
	public class PlacenoteWebAPI1 {

		private string baseURL = "https://us-central1-placenote-sdk.cloudfunctions.net/";
		private string mkey;
		private string mMapID;
		private LibPlacenote1.MapList mMapIDList;
		...
		IEnumerator GetMapListCoroutine(bool printList) {
			...
			{
				yield return www;
				WWWStatus1 ret_val = JsonConvert.DeserializeObject<WWWStatus1>(www.text);
			...
			using (WWW www = new WWW (baseURL + "searchPlaces", null, headers)) {
				...
				yield return www;
				mMapIDList = JsonConvert.DeserializeObject<LibPlacenote1.MapList> (www.text);
				mMapListUpdated = true; //tell any other co-routines waiting that map list is updated
				if (printList) {
					Debug.Log ("Available MapIDs w/ Plane Data");
					foreach (LibPlacenote1.MapInfo map in mMapIDList.places) {
		...
		IEnumerator AddMeshCoroutine(GameObject PlaneMesh, Transform MapParent=null) {
			...
			//UnityEngine.XR.iOS.SpatialCapture.CapturedPlaneUtility1.InitializePlanePrefab (PlaneMesh);
			//UnityEngine.XR.iOS.PlacenotePlaneUtility.InitializePlanePrefab (PlaneMesh);
			SpatialCapture.CapturedPlaneUtility1.InitializePlanePrefab (PlaneMesh);
			...
			foreach (var mesh in meshList.meshList) {
				//GameObject plane = UnityEngine.XR.iOS.SpatialCapture.CapturedPlaneUtility1.CreatePlaneInScene (mesh);
				//GameObject plane = UnityEngine.XR.iOS.PlacenotePlaneUtility.CreatePlaneInScene (mesh);
				GameObject plane = SpatialCapture.CapturedPlaneUtility1.CreatePlaneInScene (mesh);
				plane.transform.SetParent (MeshParent.transform);
				plane.name = "Plane " + planeNum.ToString ();
				planeNum++;
			}
			FeaturesVisualizer1.DrawMapEditor (meshList.landmarkList, MeshParent.transform);
