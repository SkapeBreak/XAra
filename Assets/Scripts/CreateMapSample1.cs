...
public class CreateMapSample1 : MonoBehaviour, PlacenoteListener1
{
	...
	void Start()
    {
		...
		StartARKit();
		FeaturesVisualizer1.EnablePointcloud(); // Optional - to see the debug features
	...
	public void OnMapSelected(LibPlacenote1.MapInfo mapInfo)
	{
	...
	public void OnStatusChange(LibPlacenote1.MappingStatus prevStatus, LibPlacenote1.MappingStatus currStatus)
	{
		...
		if (currStatus == LibPlacenote1.MappingStatus.RUNNING && prevStatus == LibPlacenote1.MappingStatus.LOST)
		{
			if(!localizeFirstTime)
			{
				...
				FeaturesVisualizer1.DisablePointcloud(); //if player is doing navigation, disable point cloud
				LoadDestinationList();
				scanPopup.SetActive(false);
		...
		else if (currStatus == LibPlacenote1.MappingStatus.RUNNING && prevStatus == LibPlacenote1.MappingStatus.WAITING)
		{
			...
			FeaturesVisualizer1.EnablePointcloud(); //if mapping enable point cloud to be see
			mExitButton.SetActive(true);
		}
