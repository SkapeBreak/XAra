...
/// </summary>
public class UnityARImageFrameData1
{
...
public class LibPlacenote1 : MonoBehaviour
{
	/// <summary>
	...
	private static LibPlacenote1 sInstance;
	private List<PlacenoteListener1> listeners = new List<PlacenoteListener1> ();
	...
	private bool libPlacenote1SessionRunning = false;

	/// <summary>
	/// Get accessor for the LibPlacenote singleton
	/// </summary>
	/// <value>The singleton instance</value>
	public static LibPlacenote1 Instance {
		get {
			return sInstance;
		}
	}
	...
	void Update () {
		...
		if (mFrameUpdated && libPlacenote1SessionRunning) {
			mFrameUpdated = false;
			if (mImage == null) {
				InitARFrameBuffer ();
			}
	...
	public void RegisterListener (PlacenoteListener1 listener)
	{
	...
	public void RemoveListener (PlacenoteListener1 listener)
	{
	...
	/// </param>
	public void SendARFrame (UnityARImageFrameData1 frameData, Vector3 position, Quaternion rotation, int screenOrientation)
	{
		...
		rotation = PNUtility1.MatrixOps.QuaternionFromMatrix (rotationMat);
	...
	{
		Matrix4x4 outputPoseMat = PNUtility1.MatrixOps.PNPose2Matrix4x4 (outputPose);
		Matrix4x4 arkitPoseMat = PNUtility1.MatrixOps.PNPose2Matrix4x4 (arkitPose);

		MappingStatus status = Instance.GetStatus();

		var listeners = Instance.listeners;
		if (status == MappingStatus.RUNNING) {
			MainThreadTaskQueue1.InvokeOnMainThread (() => {
	...
	public void StartSession (bool extend = false)
	{
		...
		libPlacenote1SessionRunning = true;
		#else
	...
	public void StopSession ()
	{
		...
		#if !UNITY_EDITOR
		PNStopSession ();
		libPlacenote1SessionRunning = false;
		#else
